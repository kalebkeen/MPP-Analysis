...
def run_synthesis(context, output_root, stage_j_dir):
    ...
    prompt = SYNTHESIS_PROMPT.format(
        ...
        # default=str on every dump: stage tables carry datetime.date /
        # pd.Timestamp cells (e.g. Stage H package actual_start/finish), which
        # json.dumps rejects otherwise. First hit on the first project with a
        # real buyout scope — an empty-buyout project never exercises it.
        top_resources=json.dumps(context["top_controlling_resources"], indent=2, default=str),
        controlling_timeline=json.dumps(context["controlling_timeline"], indent=2, default=str),
        ledger_totals=json.dumps(context["delay_ledger_totals"], indent=2, default=str),
        bucket_summary=json.dumps(context["bucket_summary"], indent=2, default=str),
        top_tasks=json.dumps(context["top_variance_tasks"], indent=2, default=str),
        per_building_top=json.dumps(context["per_building_top"], indent=2, default=str),
        appendix_row_count=context["appendix_row_count"],
        building_turnover=json.dumps(context["building_turnover"], indent=2, default=str),
        float_bands=json.dumps(context["float_bands"], indent=2, default=str),
        float_summary=json.dumps(context["float_summary"], indent=2, default=str),
        comp_range=json.dumps(context["completion_range"], indent=2, default=str),
        buyout_summary=json.dumps(context["buyout_summary"], indent=2, default=str),
        buyout_stage_breakdown=json.dumps(context["buyout_stage_breakdown"], indent=2, default=str),
        top_packages=json.dumps(context["top_packages"], indent=2, default=str),
