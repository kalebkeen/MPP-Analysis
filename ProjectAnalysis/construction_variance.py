...
def build_construction_variance(tasks_df: pd.DataFrame, cfg: dict):
    ...
    # ── Scope to construction (everything NOT under a buyout branch) ──────
    df = tasks_df[~tasks_df["outline_number"].apply(
        lambda s: is_buyout_outline(s, buyout_prefixes))].copy()

    # Use the construction-specific baseline columns (which MS Project baseline
    # slot they came from is set by schedule.construction_baseline_number at
    # extraction time). Fall back to the generic baseline_* columns for parquet
    # files written by an older build that lacks the per-section columns.
    for _f in ("start", "finish", "duration"):
        _src = f"construction_baseline_{_f}"
        if _src in df.columns:
            df[f"baseline_{_f}"] = df[_src]
