...
def start_jvm(mpxj_jar_path: str | None = None, java_home: str | None = None,
    ...
    If java_home is set, it pins which JVM JPype loads. Left to its own
    ...
    if mpxj_jar_path is None:
        try:
            ...
            if not jar.exists():
                # the pip package keeps the jars in a lib/ subfolder
                lib_jar = Path(mpxj_pkg.__file__).parent / "lib" / "mpxj.jar"
                if lib_jar.exists():
                    jar = lib_jar
            if not jar.exists():
    ...
    # MPXJ depends on many other jars (Apache POI for native .mpp binaries,
    # jackcess, sqlite-jdbc, jackson, jaxb, ...) which all ship in the same
    # lib/ folder. Putting ONLY mpxj.jar on the classpath causes
    # NoClassDefFoundError (e.g. POIFSFileSystem) and JPype "Java class has
    # no constructors" failures when a reader's dependency can't be loaded.
    # So put every jar in that folder on the classpath.
    classpath = [mpxj_jar_path]
    jar_dir = Path(mpxj_jar_path).parent
...
def read_baseline(task, n: int):
    """Return (start, finish, duration_workdays) for baseline number n.

    MS Project has 11 baseline slots: the primary "Baseline" (n=0, accessed via
    the no-arg getters) plus Baseline1..Baseline10 (the int-arg getters). Real
    projects don't use them uniformly - e.g. construction work may be baselined
    into Baseline1 while procurement sits in Baseline0 - so which slot to read
    is configurable per analysis section.
    """
    try:
        if n and n > 0:
            bs = task.getBaselineStart(n)
            bf = task.getBaselineFinish(n)
            bd = task.getBaselineDuration(n)
        else:
            bs = task.getBaselineStart()
            bf = task.getBaselineFinish()
            bd = task.getBaselineDuration()
        return to_pydate(bs), to_pydate(bf), to_working_days(bd)
    except Exception:
...
def extract_file(xml_path: Path, snapshot_label: str,
                 buyout_baseline_number: int = 0,
                 construction_baseline_number: int = 0) -> tuple[pd.DataFrame, pd.DataFrame]:
    """
    ...
    buyout_baseline_number / construction_baseline_number select which MS Project
    baseline slot (0 = primary "Baseline", 1-10 = Baseline1..Baseline10) is
    captured into the buyout_baseline_* / construction_baseline_* columns.
    """
    ...
    try:
        ...
        status_date = to_pydate(props.getStatusDate()) if props is not None else None
    except Exception:
        status_date = None
    ...
    for task in project.getTasks():
        ...
        # ── Baseline fields ─────────────────────────────────────────────
        # Generic baseline_* keeps Baseline0 (back-compat + default consumers).
        # Per-section copies let buyout (Stage H) and construction (Stage E)
        # each read whichever baseline slot actually holds their data.
        baseline_start, baseline_finish, baseline_duration = read_baseline(task, 0)
        by_bstart, by_bfinish, by_bdur = read_baseline(task, buyout_baseline_number)
        co_bstart, co_bfinish, co_bdur = read_baseline(task, construction_baseline_number)
        ...
        tasks_rows.append({
            ...
            "buyout_baseline_start":        by_bstart,
            "buyout_baseline_finish":       by_bfinish,
            "buyout_baseline_duration":     by_bdur,
            "construction_baseline_start":  co_bstart,
            "construction_baseline_finish": co_bfinish,
            "construction_baseline_duration": co_bdur,
        ...
        # task.getPredecessors() returns org.mpxj.Relation objects where THIS
        # task is the successor; the related (predecessor) task is reached via
        # getPredecessorTask(). MPXJ v16 renamed the old getTask() to the
        # explicit getPredecessorTask()/getSuccessorTask() pair.
        for pred in task.getPredecessors():
            pred_uid      = int(pred.getUniqueID()) if pred.getUniqueID() else None
            pred_task_obj = pred.getPredecessorTask()
            pred_task_id  = int(pred_task_obj.getUniqueID()) if pred_task_obj and pred_task_obj.getUniqueID() else None
    ...
    # stamp every row with the snapshot's status date (cheap — parquet RLE-encodes it)
    tasks_df["status_date"] = status_date
...
def validate_snapshot(df: pd.DataFrame, snapshot_label: str) -> dict:
    ...
    status_date = None
    if "status_date" in df.columns and len(df) and pd.notna(df["status_date"].iloc[0]):
        status_date = str(df["status_date"].iloc[0])
...
def main():
    ...
    buyout_bn       = int(sched_cfg.get("buyout_baseline_number", 0) or 0)
    construction_bn = int(sched_cfg.get("construction_baseline_number", 0) or 0)
    ...
    for i, xml_path in enumerate(xml_files, start=1):
        ...
        # Skip if already extracted and --force not set - but only if the
        # cached parquet already carries the current column schema. A cache
        # written by an older build won't have the per-section baseline columns,
        ...
        if snap_out.exists() and not args.force:
            ...
            if required_cols.issubset(existing.columns):
                ...
                continue
            print("(cache stale, re-extracting)", end="  ")
